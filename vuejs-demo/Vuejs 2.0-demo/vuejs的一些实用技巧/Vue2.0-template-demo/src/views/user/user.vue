<template>
  <div id="user">
        ...
        <div class="content">
              ...
              <router-link tag='div' v-if="!getloginState" class='login' :to="{name:'login'}" >你还没登录，请先登录</router-link> 

              <span style='text-align:center;font-size:20px;' v-else>您好，{{getUserInfo.userName}} {{getUserInfo.tenantId}}</span>

              <span @click='loginOut' v-show='getloginState' style='font-size:20px;display:inline-block;margin-left:20px;'>退出登录</span>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            ...mapGetters(['getloginState','getUserInfo'])
        },
        methods: {
            ...mapActions(['delete_user','loginOut']),
            addUser() {
