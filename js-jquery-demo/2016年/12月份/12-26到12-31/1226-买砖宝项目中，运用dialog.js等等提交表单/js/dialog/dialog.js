...
function showDialog_(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	if (mode == 'js'){
		eval(func);
		hideMenu(null, 'dialog');
		return ;
...
function showDialog_tel_kefu(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
    if (mode == 'js'){
        eval(func);
        hideMenu(null, 'dialog');
        return ;
    ...
    var s = hidedom + '<h3 class="dialog_head"><span class="dialog_title">';
    s += t ? t : '<span class="dialog_title_icon">电话客服</span>';
    s += '</span><span class="dialog_close_button" id="fwin_dialog_close" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="关闭">X</span></h3>';
    if(mode == 'info') {
        s += msg ? msg : '';
    } else {
        s += '<div class="eject_con"><div class="dialog_message_contents">';
        s += '<i class="i_kefu"></i>' + msg + '</div></div>';
        s += '<div class="dialog_buttons_bar">' + (leftmsg ? '<time class="countdown">'+'<i class="icon-time"></i>' + leftmsg + '</time>' : '') + (showconfirm ? '<a href="javascript:void(0)" id="fwin_dialog_submit" class="dialog-bottom-btn dialog-bottom-btn mr10">'+confirmtxt+'</a>' : '');
        s += mode == 'confirm' ? '<a href="javascript:void(0)" id="fwin_dialog_cancel" class="dialog-bottom-btn" onclick="hideMenu(\'' + menuid + '\', \'dialog\')">'+canceltxt+'</a>' : '';
        s += '</div>';
    }
    ...
    if($$('fwin_dialog_submit')) {
        $$('fwin_dialog_submit').onclick = function() {
            ...
            hideMenu(menuid, 'dialog');
             $(".index_zhezhao").stop(true).hide();
         };
        $$('fwin_dialog_close').onclick = $$('fwin_dialog_submit').onclick;
    }
...
function showDialog_tel_new(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
    if (mode == 'js'){
        eval(func);
        hideMenu(null, 'dialog');
        return ;
    ...
    var s = hidedom + '<h3 class="dialog_head"><span class="dialog_title">';
    s += t ? t : '<span class="dialog_title_icon">友情提醒</span>';
    s += '</span><span class="dialog_close_button" id="fwin_dialog_close" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="关闭">X</span></h3>';
    if(mode == 'info') {
        s += msg ? msg : '';
    } else {
...
function ajax_get_confirm(msg, url, msg_){
	if (msg != ''){
		if(msg_ !== '' && msg_){
			showDialog_(msg_, 'confirm', '', function(){showDialog(msg, 'confirm', '', function(){ajaxget(url);});});
		}else{
