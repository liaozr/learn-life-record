    ...
    <div class="section section9 newlist">
        ...
        <div class="section_box content">
			...
			<van-list
			  ...
			  @load="onLoad"
			>
			  <div @click="gotoDetail(item)" v-for="(item,index) in newList" :key="item.id" class="list flex-center-between" :class="{'lastlist':index == newList.length-1}">
			     <div class="img">
...
export default {
  ...
  methods: {
	onLoad() {
	  console.log('come from onload事件')
	  this.getNewsList()
	},  
    gotoDetail(item) {
      this.$router.push({
