<template>
	<view class="content myTasks">
		...
		<view class="tabhead">
			<view @click="index=0" class="list">
				<view class="tab" :class="{active:index===0}">
					列表
				</view>
			</view>
			<view @click="index=1" class="list">
				<view class="tab" :class="{active:index===1}">
					地图
				</view>
			</view>
		</view>
		<view class="flex1" style="overflow: hidden;">
			<scroll-view scroll-y="" v-if="index==0" class="itemlist" :key="1">
				<view @click="gotodetail(item)" v-for="(item,index) in superviseList" class="list" :key="index">
					<view class="title">
			...
			<map-view v-else="" :key="2"></map-view>
		</view>
	</view>
</template>

<script>
	import mapView from '@/pages/zhandianMsg/map';
