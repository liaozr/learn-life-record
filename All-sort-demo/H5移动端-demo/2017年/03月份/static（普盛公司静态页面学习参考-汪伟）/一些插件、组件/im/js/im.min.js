...
var SDKBridge=function(g,G,h,E,s){var n=this;this.person={};this.team=[];this.person[E]=true;this.controller=g;this.cache=G;var l={getUser:function(H){return n.cache.getUserById(H)},getSession:function(H){return n.cache.findSession(H)},getMsg:function(H){return n.nim.findMsg(n.cache.msgs[H.sessionId],H.idClient)},getSysMsg:function(H){return n.nim.findSysMsg(n.cache.sysMsgs,H.idServer)}};this.nim=new NIM({appKey:h,account:E,token:s,onconnect:j.bind(this),ondisconnect:v.bind(this),onerror:u.bind(this),onwillreconnect:z.bind(this),onloginportschange:r.bind(this),onteams:A.bind(this),onteammembers:m.bind(this),onmsg:p.bind(this),onroamingmsgs:o.bind(this),onofflinemsgs:o.bind(this),onsessions:y.bind(this),onupdatesession:C.bind(this),onsyncteammembersdone:t.bind(this),onsyncdone:d.bind(this),onmyinfo:i.bind(this),onupdatemyinfo:i.bind(this),onsysmsg:c.bind(this,1),onofflinesysmsgs:a.bind(this),onupdatesysmsg:c.bind(this,0),oncustomsysmsg:B.bind(this),onofflinecustomsysmsgs:w.bind(this),onmutelist:k.bind(this),onblacklist:D.bind(this),onsynccreateteam:x.bind(this),onsyncmarkinblacklist:q.bind(this),onsyncmarkinmutelist:F.bind(this),onsyncfriendaction:e.bind(this),dataSource:l});function j(){this.teamMemberDone=false;this.sysMsgDone=false;console.log("用户连接成功")}function f(H){this.iskicked=true}function z(H){$("#j-errorNetwork").removeClass("hide")}function u(H){console.log("错误信息"+H)}function v(H){var I=this;console.log("连接断开");if(H){switch(H.code){case 302:$.core.showErrorInfo(H.message);top.$("#chatWindow").data("kendoWindow").close();break;case"kicked":var J={PC:"电脑版",Web:"网页版",Android:"手机版",iOS:"手机版",WindowsPhone:"手机版"};var K=H.from;$.core.showErrorInfo("您的IM帐号于"+dateFormat(+new Date(),"HH:mm")+"被"+(J[K]||"其他端")+"踢出下线，请确定帐号信息安全!");top.$("#chatWindow").data("kendoWindow").close();break;default:break}}}function r(H){console.log("当前登录帐号在其它端的状态发生改变了",H);this.controller.loginPorts(H)}function A(I){var H=this.cache.getTeamlist();H=this.nim.mergeTeams(H,I);H=this.nim.cutTeams(H,I.invalid);this.cache.setTeamList(H)}function b(H){var I=this.cache.getFriends();I=this.nim.mergeFriends(I,H);I=this.nim.cutFriends(I,H.invalid);this.cache.setFriends(I);for(var J=0;J<I.length;J++){this.person[I[J].account]=true}}function y(H){var I=this.cache.getSessions();this.cache.setSessions(this.nim.mergeSessions(I,H));for(var J=0;J<H.length;J++){if(H[J].scene==="p2p"){this.person[H[J].to]=true}else{this.team.push(H[J].to)}}}function C(I){var H=this.cache.getSessions();this.cache.setSessions(this.nim.mergeSessions(H,I));this.controller.buildConversations()}function o(I){I=I.msgs;this.cache.addMsgs(I);for(var H=0;H<I.length;H++){if(I[H].scene==="p2p"){this.person[I[H].from!==E?I[H].from:I[H].to]=true}}}function d(){console.log("消息同步完成");var H=this.controller;this.sysMsgDone=true;if(this.teamMemberDone){H.initInfo(this.person,this.team)}}function t(){console.log("群成员同步完成");var H=this.controller;this.teamMemberDone=true;if(this.sysMsgDone){H.initInfo(this.person,this.team)}}function m(H){this.cache.setTeamMembers(H.teamId,H.members);var I=H.members;for(var J=0;J<I.length;J++){this.person[I[J].account]=true}}function p(H){this.controller.doMsg(H)}function a(I){var H=this.cache.getSysMsgs();H=this.nim.mergeSysMsgs(H,I).sort(function(K,J){return J.time-K.time});this.cache.setSysMsgs(H);this.cache.addSysMsgCount(H.length)}function c(K,I){var L=I.type,H=this.controller,J=this.cache;G=J.getSysMsgs();G=this.nim.mergeSysMsgs(G,I).sort(function(N,M){return M.time-N.time});this.cache.setSysMsgs(G);if(I.category!=="team"){switch(L){case"deleteFriend":J.removeFriend(I.from);H.buildContacts();break;case"addFriend":if(!this.cache.getUserById(I.from)){this.getUser(I.from,function(M,N){if(!M){J.addFriend(N);J.updatePersonlist(N);H.buildContacts()}})}else{J.addFriend(I.friend);H.buildContacts()}break;default:console.log("系统消息---->"+I);break}}else{if(K){this.cache.addSysMsgCount(1);H.showSysMsgCount()}H.buildSysNotice()}}function B(I){var H=this.controller;this.cache.addCustomSysMsgs([I]);this.cache.addSysMsgCount(1);H.showSysMsgCount()}function w(H){this.cache.addCustomSysMsgs(H);this.cache.addSysMsgCount(H.length)}function D(I){var H=this.cache.getBlacklist();H=this.nim.mergeRelations(H,I);H=this.nim.cutRelations(H,I.invalid);this.cache.setBlacklist(H);for(var J=0;J<G.length;J++){this.person[G[J]]=true}}function k(H){var J=this.cache.getMutelist();J=this.nim.mergeRelations(J,H);J=this.nim.cutRelations(J,H.invalid);this.cache.setMutelist(J);for(var I=0;I<G.length;I++){this.person[G[I]]=true}}function i(H){this.cache.updatePersonlist(H);this.controller.showMe()}function x(H){this.cache.addTeam(H);this.controller.buildTeams()}function e(H){g.doSyncFriendAction(H)}function q(H){if(H.isAdd){this.cache.addToBlacklist(H.record)}else{this.cache.removeFromBlacklist(H.account)}this.controller.buildConversations();this.controller.buildContacts()}function F(H){if(H.isAdd){this.cache.addToMutelist(H.record)}else{this.cache.removeFromMutelist(H.account)}}};SDKBridge.prototype.setCurrSession=function(a,b){this.nim.setCurrSession(a+"-"+b)};SDKBridge.prototype.sendTextMessage=function(a,d,b,c){this.nim.sendText({scene:a||"p2p",to:d,text:b,done:c})};SDKBridge.prototype.sendCustomMessage=function(b,d,a,c){this.nim.sendCustomMsg({scene:b||"p2p",to:d,content:JSON.stringify(a),done:c})};SDKBridge.prototype.sendFileMessage=function(f,h,d,g){var c=this,e=d.value,b=e.substring(e.lastIndexOf(".")+1,e.length),a=/png|jpg|bmp|jpeg|gif/i.test(b)?"image":"file";this.nim.sendFile({scene:f,to:h,type:a,fileInput:d,uploadprogress:function(i){console&&console.log(i.percentageText)},uploaderror:function(){console&&console.log("上传失败")},uploaddone:function(i){console&&console.log("上传完成，服务器处理中...")},beforesend:function(i){console&&console.log("正在发送消息, id="+i)},done:g})};SDKBridge.prototype.getHistoryMsgs=function(a){this.nim.getHistoryMsgs(a)};SDKBridge.prototype.getLocalMsgs=function(c,d,b,a){if(b){this.nim.getLocalMsgs({scene:c,to:d,lastMsgIdClient:b,limit:20,done:a})}else{this.nim.getLocalMsgs({scene:c,to:d,limit:20,done:a})}};SDKBridge.prototype.getLocalTeams=function(b,a){this.nim.getLocalTeams({teamIds:b,done:a})};SDKBridge.prototype.getLocalSysMsgs=function(a){this.nim.getLocalSysMsgs({done:a})};SDKBridge.prototype.deleteAllLocalSysMsgs=function(a){this.nim.deleteAllLocalSysMsgs({done:a})};SDKBridge.prototype.passTeamApply=function(a,b,c){this.nim.passTeamApply({teamId:a,from:b,idServer:c,done:function(d,e){}})};SDKBridge.prototype.rejectTeamApply=function(a,b,c){this.nim.rejectTeamApply({teamId:a,from:b,idServer:c,done:function(d,e){}})};SDKBridge.prototype.rejectTeamInvite=function(a,b,c){this.nim.rejectTeamInvite({teamId:a,from:b,idServer:c,done:function(d,e){}})};SDKBridge.prototype.acceptTeamInvite=function(a,b,c){this.nim.acceptTeamInvite({teamId:a,from:b,idServer:c,done:function(d,e){}})};SDKBridge.prototype.kick=function(a){var b=(a===0?this.mobileDeviceId:this.pcDeviceId);this.nim.kick({deviceIds:[b],done:function(c,d){$.core.showErrorInfo("踢"+(a===0?"移动":"PC")+"端"+(!c?"成功":"失败"));console.log(c);console.log(d)}})};SDKBridge.prototype.getTeam=function(b,a){this.nim.getTeam({teamId:b,done:a})};SDKBridge.prototype.applyTeam=function(a){this.nim.applyTeam({teamId:a,done:function(b,c){if(b){alert(b.message)}else{alert("入群申请已发出")}}})};SDKBridge.prototype.createTeam=function(a){this.nim.createTeam(a)};SDKBridge.prototype.getTeamMembers=function(a){this.nim.getTeamMembers(a)};SDKBridge.prototype.updateTeam=function(a){this.nim.updateTeam(a)};SDKBridge.prototype.leaveTeam=function(a){this.nim.leaveTeam(a)};SDKBridge.prototype.dismissTeam=function(a){this.nim.dismissTeam(a)};SDKBridge.prototype.addTeamMembers=function(a){this.nim.addTeamMembers(a)};SDKBridge.prototype.removeTeamMembers=function(a){this.nim.removeTeamMembers(a)};SDKBridge.prototype.addFriend=function(a,b){this.nim.addFriend({account:a,done:b})};SDKBridge.prototype.deleteFriend=function(a,b){this.nim.deleteFriend({account:a,done:b})};SDKBridge.prototype.markInMutelist=function(a,c,b){this.nim.markInMutelist({account:a,isAdd:c,done:b})};SDKBridge.prototype.markInBlacklist=function(a,c,b){this.nim.markInBlacklist({account:a,isAdd:c,done:b})};SDKBridge.prototype.getUsers=function(a,b){this.nim.getUsers({accounts:a,done:b})};SDKBridge.prototype.getUser=function(a,b){this.nim.getUser({account:a,done:b})};SDKBridge.prototype.updateMyInfo=function(a,e,g,d,c,b,f){this.nim.updateMyInfo({nick:a,gender:e,birth:g,tel:d,email:c,sign:b,done:f})};SDKBridge.prototype.updateMyAvatar=function(a,b){this.nim.updateMyInfo({avatar:a,done:b})};SDKBridge.prototype.updateFriend=function(a,c,b){this.nim.updateFriend({account:a,alias:c,done:b})};SDKBridge.prototype.previewImage=function(a){this.nim.previewFile({type:"image",fileInput:a.fileInput,uploadprogress:function(b){console.log("文件总大小: "+b.total+"bytes");console.log("已经上传的大小: "+b.loaded+"bytes");console.log("上传进度: "+b.percentage);console.log("上传进度文本: "+b.percentageText)},done:a.callback})};
var Cache=(function(){var b=function(a){this.friendslist=[];this.personlist={};this.teamlist=[];this.teamMembers={};this.teamMap={};this.msgs={};this.sessions=[];this.blacklist=[];this.mutelist=[];this.sysMsgs=[];this.customSysMsgs=[];this.sysMsgCount=0};b.prototype.getUserById=function(a){if(this.personlist[a]){return this.personlist[a]}return false};b.prototype.setPersonlist=function(e){var f;for(var a=e.length-1;a>=0;a--){f=e[a];this.personlist[f.account]=f}};b.prototype.updateAvatar=function(a){this.personlist[userUID].avatar=a};b.prototype.updatePersonlist=function(a){if(!this.personlist[a.account]){this.personlist[a.account]=a}else{for(var d in a){this.personlist[a.account][d]=a[d]}}};b.prototype.getPersonlist=function(){return this.personlist};b.prototype.setFriends=function(a){this.friendslist=a};b.prototype.getFriends=function(a){return this.friendslist};b.prototype.getFriendAlias=function(d){for(var a=this.friendslist.length-1;a>=0;a--){if(this.friendslist[a].account==d){return this.friendslist[a].alias||""}}};b.prototype.updateFriendAlias=function(e,f){for(var a=this.friendslist.length-1;a>=0;a--){if(this.friendslist[a].account==e){this.friendslist[a].alias=f;return}}};b.prototype.addFriend=function(a){if(!this.isFriend(a.account)){this.friendslist.push(a)}};b.prototype.removeFriend=function(d){for(var a=this.friendslist.length-1;a>=0;a--){if(this.friendslist[a].account==d){this.friendslist.splice(a,1)}}};b.prototype.getFriendslist=function(){var d=[];for(var a=0;a<this.friendslist.length;a++){d.push(this.getUserById(this.friendslist[a].account))}return d};b.prototype.getFriendslistOnShow=function(){var d=[];for(var a=0;a<this.friendslist.length;a++){if(!this.inBlacklist(this.friendslist[a].account)){d.push(this.getUserById(this.friendslist[a].account))}}return d};b.prototype.isFriend=function(d){for(var a=this.friendslist.length-1;a>=0;a--){if(this.friendslist[a].account==d){return true}}return false};b.prototype.setSessions=function(a){this.sessions=a};b.prototype.getSessions=function(){return this.sessions};b.prototype.findSession=function(d){for(var a=this.sessions.length-1;a>=0;a--){if(this.sessions[a].id===d){return this.sessions[a]}}return false};b.prototype.addMsgs=function(f){var g,a;if(!$.isArray(f)){this.addMsg(f);return}for(var h=0;h<f.length;h++){if(f[h].scene==="team"){a=f[h].to;if(!this.msgs["team-"+a]){this.msgs["team-"+a]=[]}this.msgs["team-"+a].push(f[h])}else{a=(f[h].from===userUID?f[h].to:f[h].from);if(!this.msgs["p2p-"+a]){this.msgs["p2p-"+a]=[]}this.msgs["p2p-"+a].push(f[h])}}};b.prototype.addMsg=function(e){var a;if(e.scene==="team"){a="team-"+e.to;if(!this.msgs[a]){this.msgs[a]=[]}this.msgs[a].push(e)}else{a="p2p-"+(e.from===userUID?e.to:e.from);if(!this.msgs[a]){this.msgs[a]=[]}this.msgs[a].push(e)}for(var f=0;f<this.sessions.length;f++){if(a===this.sessions[f]){this.sessions.splice(f,1);break}}};b.prototype.addMsgsByReverse=function(f){var g,a;for(var h=0;h<f.length;h++){if(f[h].scene==="team"){a=f[h].to;if(!this.msgs["team-"+a]){this.msgs["team-"+a]=[]}this.msgs["team-"+a].unshift(f[h])}else{a=(f[h].from===userUID?f[h].to:f[h].from);if(!this.msgs["p2p-"+a]){this.msgs["p2p-"+a]=[]}this.msgs["p2p-"+a].unshift(f[h])}}};b.prototype.setSysMsgs=function(a){this.sysMsgs=a};b.prototype.getSysMsgs=function(a){return this.sysMsgs};b.prototype.addCustomSysMsgs=function(d){for(var a=0;a<d.length;a++){this.customSysMsgs.push(d[a])}};b.prototype.deleteCustomSysMsgs=function(){this.customSysMsgs=[]};b.prototype.getCustomSysMsgs=function(a){return this.customSysMsgs};b.prototype.getSysMsgCount=function(a){return this.sysMsgCount};b.prototype.setSysMsgCount=function(a){this.sysMsgCount=a};b.prototype.addSysMsgCount=function(a){this.sysMsgCount=this.sysMsgCount+a};b.prototype.getMsgs=function(a){if(!!this.msgs[a]){return this.msgs[a]}else{return[]}};b.prototype.getMsgsByUser=function(a){return this.msgs[a]||[]};b.prototype.addOfflineMsgs=function(d){for(var a=d.length-1;a>=0;a--){if(/text|image|file|audio|video|geo|custom|notification/i.test(d[a].type)){this.addMsgs(d[a])}else{continue}}};b.prototype.setTeamList=function(e){var f;for(var a=e.length-1;a>=0;a--){f=e[a];this.teamMap[f.teamId]=f}this.teamlist=e};b.prototype.addTeam=function(a){if(!this.hasTeam(a.teamId)){this.teamMap[a.teamId]=a;this.teamlist.push(a)}};b.prototype.hasTeam=function(e){var f;for(var a=this.teamlist.length-1;a>=0;a--){f=this.teamlist[a];if(f.teamId===e){return true}}return false};b.prototype.getTeamlist=function(){return this.teamlist};b.prototype.getTeamMap=function(){return this.teamMap};b.prototype.addTeamMap=function(d){for(var a=d.length-1;a>=0;a--){item=d[a];this.teamMap[item.teamId]=item}};b.prototype.getTeamById=function(a){if(this.hasTeam(a)){return this.teamMap[a]}return null};b.prototype.getTeamMapById=function(a){return this.teamMap[a]||null};b.prototype.removeTeamById=function(d){for(var a in this.teamlist){if(this.teamlist[a].teamId===d){this.teamlist.splice(a,1);break}}};b.prototype.updateTeam=function(f,g){for(var h in g){this.teamMap[f][h]=g[h]}for(var a in this.teamlist){if(this.teamlist[a].teamId===f){for(var h in g){this.teamlist[a][h]=g[h]}break}}};b.prototype.setTeamMembers=function(d,a){this.teamMembers[d]=a};b.prototype.getTeamMembers=function(a){return this.teamMembers[a]||[]};b.prototype.setMutelist=function(a){this.mutelist=a};b.prototype.getMutelist=function(a){return this.mutelist};b.prototype.inMutelist=function(d){for(var a=this.mutelist.length-1;a>=0;a--){if(this.mutelist[a].account==d){return true}}return false};b.prototype.addToMutelist=function(a){this.mutelist.push(a)};b.prototype.removeFromMutelist=function(d){for(var a=this.mutelist.length-1;a>=0;a--){if(this.mutelist[a].account==d){this.mutelist.splice(a,1);return true}}return false};b.prototype.setBlacklist=function(a){this.blacklist=a};b.prototype.getBlacklist=function(){return this.blacklist};b.prototype.inBlacklist=function(d){for(var a=this.blacklist.length-1;a>=0;a--){if(this.blacklist[a].account==d){return true}}return false};b.prototype.addToBlacklist=function(a){this.blacklist.push(a)};b.prototype.removeFromBlacklist=function(d){for(var a=this.blacklist.length-1;a>=0;a--){if(this.blacklist[a].account==d){this.blacklist.splice(a,1);return true}}return false};return b})();
if(!Function.prototype.bind){Function.prototype.bind=function(){var d=this,e=Array.prototype.slice.call(arguments),f=e.shift();return function(){return d.apply(f,e.concat(Array.prototype.slice.call(arguments)))}}}var _$encode=function(c,d){d=""+d;if(!c||!d){return d||""}return d.replace(c.r,function(b){var a=c[!c.i?b.toLowerCase():b];return a!=null?a:b})};var _$escape=(function(){var d=/<br\/?>$/,c={r:/\<|\>|\&|\r|\n|\s|\'|\"/g,"<":"&lt;",">":"&gt;","&":"&amp;"," ":"&nbsp;",'"':"&quot;","'":"&#39;","\n":"<br/>","\r":""};return function(a){a=_$encode(c,a);return a.replace(d,"<br/><br/>")}})();Date.prototype.Format=function(f){var d={"M+":this.getMonth()+1,"d+":this.getDate(),"h+":this.getHours(),"m+":this.getMinutes(),"s+":this.getSeconds(),"q+":Math.floor((this.getMonth()+3)/3),S:this.getMilliseconds()};if(/(y+)/.test(f)){f=f.replace(RegExp.$1,(this.getFullYear()+"").substr(4-RegExp.$1.length))}for(var e in d){if(new RegExp("("+e+")").test(f)){f=f.replace(RegExp.$1,(RegExp.$1.length==1)?(d[e]):(("00"+d[e]).substr((""+d[e]).length)))}}return f};Array.prototype.each=function(g){g=g||Function.K;var a=[];var j=Array.prototype.slice.call(arguments,1);for(var h=0;h<this.length;h++){var i=g.apply(this,[this[h],h].concat(j));if(i!=null){a.push(i)}}return a};Array.prototype.contains=function(c){for(var d=0;d<this.length;d++){if(this[d]==c){return true}}return false};Array.prototype.uniquelize=function(){var c=new Array();for(var d=0;d<this.length;d++){if(!c.contains(this[d])){c.push(this[d])}}return c};Array.complement=function(a,b){return Array.minus(Array.union(a,b),Array.intersect(a,b))};Array.intersect=function(a,b){return a.uniquelize().each(function(c){return b.contains(c)?c:null})};Array.minus=function(a,b){return a.uniquelize().each(function(c){return b.contains(c)?null:c})};Array.union=function(a,b){return a.concat(b).uniquelize()};Array.prototype.insert=function(d,c){this.splice(d,0,c)};function buildSessionMsg(j){var e=(j.scene!="p2p"?((j.from===userUID)?"你":getNick(j.from))+":":""),i=j.type;if(!/text|image|file|audio|video|geo|custom|notification/i.test(i)){return""}switch(i){case"text":e+=_$escape(j.text);e=buildEmoji(e);break;case"image":e+="[图片]";break;case"file":if(!/exe|bat/i.test(j.file.ext)){e+="[文件]"}else{e+="[非法文件，已被本站拦截]"}break;case"audio":e+="[语音]";break;case"video":e+="[视频]";break;case"geo":e+="[位置]";break;case"custom":try{var h=JSON.parse(j.content);if(h.type===1){e+="[猜拳]"}else{if(h.type===2){e+="[阅后即焚]"}else{if(h.type===3){e+="[贴图]"}else{if(h.type===4){e+="[白板]"}else{if(h.type===5){e+="["+h.relTypeName+"]"+h.relNo}else{e+="[自定义消息]"}}}}}}catch(g){e+="[自定义消息]"}break;case"notification":e="["+transNotification(j)+"]";break;default:e+="[未知消息类型]";break}if(j.status==="fail"){e='<i class="icon icon-error"></i>'+e}return e}function getMessage(m){var r="",o=m.file?_$escape(m.file.url):"",n=(m.from!==userUID)?"收到":"发送";switch(m.type){case"text":var p=/(http:\/\/[\w.\/]+)(?![^<]+>)/gi;r=_$escape(m.text);r=r.replace(p,"<a href='$1' target='_blank'>$1</a>");r=buildEmoji(r);r="<div class='f-maxWid'>"+r+"</div>";break;case"image":if(m.status===-1){r="<p>["+m.message.message+"]</p>"}else{var q=m.from+"-"+m.to;m.file.url=_$escape(m.file.url);r='<a class="fancybox" data-fancybox-group="'+q+'" href="'+m.file.url+'?imageView" target="_blank"><img onload="loadImg()" data-src="'+m.file.url+'" src="'+m.file.url+'?imageView&thumbnail=200x0&quality=85"/></a>'}break;case"file":if(m.status===-1){r="<p>["+m.message.message+"]</p>"}else{if(/png|jpg|bmp|jpeg|gif/i.test(m.file.ext)){m.file.url=_$escape(m.file.url);r='<a class="f-maxWid" href="'+m.file.url+'?imageView" target="_blank"><img data-src="'+m.file.url+'" src="'+m.file.url+'?imageView&thumbnail=200x0&quality=85"/></a>'}else{if(!/exe|bat/i.test(m.file.ext)){o+=m.file?"?download="+encodeURI(_$escape(m.file.name)):"";r='<a href="'+o+'" target="_blank" class="download-file f-maxWid"><span class="icon icon-file2"></span>'+_$escape(m.file.name)+"</a>"}else{r="<p>[非法文件，已被本站拦截]</p>"}}}break;case"video":r='<video src= "'+o+'" controls>您的浏览器不支持 video 标签。</video>';break;case"audio":if(!!window.Audio){if(m.from===userUID&&m.from!==m.to){r='<div class="u-audio j-mbox right"> <a href="javascript:;" class="j-play playAudio" data-dur="'+m.file.dur+'"  data-src="'+o+'">点击播放</a><b class="j-duration">'+Math.floor((m.file.dur)/1000)+'"</b><span class="u-icn u-icn-play" title="播放音频"></span></div><span class="convert icon-refresh"></span>'}else{r='<div class="u-audio j-mbox left"> <a href="javascript:;" class="j-play playAudio" data-dur="'+m.file.dur+'"  data-src="'+o+'">点击播放</a><b class="j-duration">'+Math.floor((m.file.dur)/1000)+'"</b><span class="u-icn u-icn-play" title="播放音频"></span></div><span class="convert icon-refresh"></span>'}}else{r='<a href="'+o+'" target="_blank" class="download-file"><span class="icon icon-file2"></span>['+n+"一条语音消息]</a>"}break;case"geo":var e=m.geo;r='<img class="location" data-nick="'+m.fromNick+'" data-lng="'+e.lng+'" data-lat="'+e.lat+'" src="/view/static/comp/im/images/location_bg.png"><div class="locationTitle">'+e.title+"</div>";break;case"custom":try{var s=JSON.parse(m.content);if(s.type===1){r=n+"一条[猜拳]消息,请到手机或电脑客户端查看"}else{if(s.type===2){r=n+"一条[阅后即焚]消息,请到手机或电脑客户端查看"}else{if(s.type===3){var u=_$escape(s.data.catalog),v=_$escape(s.data.chartlet);r='<img class="chartlet" onload="loadImg()" src="/view/static/comp/im/images/'+u+"/"+v+'.png">'}else{if(s.type===4){r=n+"一条[白板]消息,请到手机或电脑客户端查看"}else{if(s.type===5){r="<div class='shareMsg' onclick='showShareBill(\""+s.relType+'","'+s.relTypeName+'","'+s.relNo+"\")'><div class='shareMsg-heading'><i class='icon-link'/>"+s.relTypeName+"</div>";r+="<div class='shareMsg-body'>"+s.relNo+"</div>";r+="</div>"}else{r=n+"一条[自定义]消息，请到手机或电脑客户端查看"}}}}}}catch(t){r=n+"一条[自定义]消息，请到手机或电脑客户端查看"}break;default:r=n+"一条[未知消息类型]消息";break}return r}var transTime=(function(){var b=function(e){e.setMinutes(0);e.setSeconds(0);e.setMilliseconds(0);e.setHours(0);var a=e.getTime();e.setMonth(1);e.setDate(1);var f=e.getTime();return[a,f]};return function(d){var a=b(new Date());if(d>=a[0]){return dateFormat(d,"HH:mm")}else{if(d<a[0]&&d>=a[1]){return dateFormat(d,"MM-dd HH:mm")}else{return dateFormat(d,"yyyy-MM-dd HH:mm")}}}})();var transTime2=(function(){var b=function(e){e.setMinutes(0);e.setSeconds(0);e.setMilliseconds(0);e.setHours(0);var a=e.getTime();e.setMonth(1);e.setDate(1);var f=e.getTime();return[a,f]};return function(d){var a=b(new Date());if(d>=a[0]){return dateFormat(d,"HH:mm")}else{if(d>=a[0]-60*1000*60*24){return"昨天"}else{if(d>=(a[0]-2*60*1000*60*24)){return"前天"}else{if(d>=(a[0]-7*60*1000*60*24)){return"星期"+dateFormat(d,"w")}else{if(d>=a[1]){return dateFormat(d,"MM-dd")}else{return dateFormat(d,"yyyy-MM-dd")}}}}}}})();function buildSender(c){var d="";if(c.from===c.to){if(c.fromClientType==="Web"){d="me"}else{d="you"}}else{if(c.from===userUID&&!c.fromClientType){d="me"}else{d="you"}if(c.from===userUID&&c.to!=userUID){d="me"}}return d}var dateFormat=(function(){var l={i:!0,r:/\byyyy|yy|MM|cM|eM|M|dd|d|HH|H|mm|ms|ss|m|s|w|ct|et\b/g},i=["上午","下午"],o=["A.M.","P.M."],j=["日","一","二","三","四","五","六"],p=["一","二","三","四","五","六","七","八","九","十","十一","十二"],m=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec"];var k=function(a){a=parseInt(a)||0;return(a<10?"0":"")+a};var n=function(a){return a<12?0:1};return function(b,d,a){if(!b||!d){return""}b=new Date(b);l.yyyy=b.getFullYear();l.yy=(""+l.yyyy).substr(2);l.M=b.getMonth()+1;l.MM=k(l.M);l.eM=m[l.M-1];l.cM=p[l.M-1];l.d=b.getDate();l.dd=k(l.d);l.H=b.getHours();l.HH=k(l.H);l.m=b.getMinutes();l.mm=k(l.m);l.s=b.getSeconds();l.ss=k(l.s);l.ms=b.getMilliseconds();l.w=j[b.getDay()];var c=n(l.H);l.ct=i[c];l.et=o[c];if(!!a){l.H=l.H%12}return _$encode(l,d)}})();function transNotification(p){var s=p.attach.type,r=(p.from===userUID?true:false),t,m,o,u=[];var n=p.attach&&p.attach.team;if(!n){n=im.cache.getTeamMapById(p.target)}else{if(!n.type){n=im.cache.getTeamMapById(p.target)}}if(n.type&&n.type==="normal"){m="讨论组"}else{m="群"}switch(s){case"addTeamMembers":o=p.attach.accounts;for(var v=0;v<o.length;v++){if(o[v]===userUID){u.push("你")}else{u.push(getNick(o[v]))}}u=u.join(",");t=r?"你将"+u+"加入"+m:u+"加入"+m;return t;break;case"removeTeamMembers":o=p.attach.accounts;for(var v=0;v<o.length;v++){if(o[v]===userUID){u.push("你")}else{u.push(getNick(o[v]))}}u=u.join(",");t=r?("你将"+u+"移除"+m):(u+"被移除"+m);return t;break;case"leaveTeam":var u=(p.from===userUID)?"你":getNick(p.from);t=u+"退出了"+m;return t;break;case"updateTeam":if(p.attach.team.joinMode){switch(p.attach.team.joinMode){case"noVerify":t="群身份验证模式更新为允许任何人加入";break;case"needVerify":t="群身份验证模式更新为需要验证消息";break;case"rejectAll":t="群身份验证模式更新为不允许任何人申请加入";break;default:t="更新群消息";break}}else{if(p.attach.team.name){var i=(p.from===userUID)?"你":getNick(p.from);t=i+"更新"+m+"名称为"+p.attach.team.name}else{if(p.attach.team.intro){var i=(p.from===userUID)?"你":getNick(p.from);t=i+"更新群介绍为"+p.attach.team.intro}else{t="更新群消息"}}}return t;break;case"acceptTeamInvite":var u,q;if(p.from===p.attach.account){u=(p.from===userUID)?"你":getNick(p.from);t=u?u:p.from+"加入了群"}else{q=(p.attach.account===userUID)?"你":getNick(p.attach.account);u=(p.from===userUID)?"你":getNick(p.from);t=u+"接受了"+q+"的入群邀请"}return t;break;case"passTeamApply":var u,q;if(p.from===p.attach.account){u=(p.from===userUID)?"你":getNick(p.from);t=u+"加入了群"}else{u=(p.attach.account===userUID)?"你":getNick(p.attach.account);q=(p.from===userUID)?"你":getNick(p.from);t=q+"通过了"+u+"的入群申请"}return t;break;case"dismissTeam":var u=(p.from===userUID)?"你":getNick(p.from);t=u+"解散了群";return t;break;default:return"通知消息";break}}function removeChatVernier(b){if(b==$("li.active").attr("data-account")){$("#j-chatVernier span").css("top","-20px")}}function loadImg(){$("#j-chatContent").scrollTop(99999)}function getAvatar(f,d){if(!f||f==""){if(d==2){return ctx+"/view/static/comp/im/images/felman.png"}return ctx+"/view/static/comp/im/images/man.png"}var e=/^((http|https|ftp):\/\/)?(\w(\:\w)?@)?([0-9a-z_-]+\.)*?([a-z0-9-]+\.[a-z]{2,6}(\.[a-z]{2})?(\:[0-9]{2,6})?)((\/[^?#<>\/\\*":]*)+(\?[^#]*)?(#.*)?)?$/i;if(e.test(f)){return f+"?imageView&thumbnail=80x80&quality=85"}else{return f}}function getNick(c,d){d=d||im.cache;return d.getFriendAlias(c)||(d.getUserById(c)?d.getUserById(c).nick:c)};
(function webpackUniversalModuleDefinition(d,c){if(typeof exports==="object"&&typeof module==="object"){module.exports=c()}else{if(typeof define==="function"&&define.amd){define([],c)}else{if(typeof exports==="object"){exports.NIMUIKit=c()}else{d.NIMUIKit=c()}}}})(this,function(){return(function(f){var e={};function d(a){if(e[a]){return e[a].exports}var b=e[a]={exports:{},id:a,loaded:false};f[a].call(b.exports,b,b.exports,d);b.loaded=true;return b.exports}d.m=f;d.c=e;d.p="";return d(0)})([function(h,f,g){var e={};e.SessionList=g(1);e.FriendList=g(3);e.TeamList=g(4);h.exports=e},function(g,h,f){var i=f(2);var j=function(b){var e=b.parent,d=b.data,c=b.onclickitem||function(m,n){console.log("account:"+m+"---type:"+n)},a=b.onclickavatar||function(m,n){console.log("account:"+m+"---type:"+n)};this._body=document.createElement("ul");this._body.className=b.clazz||"m-panel j-session";this.provider=b.infoprovider;i.addEvent(this._body,"click",function(u){var s=this,t=u||window.event,x,v,w=t.srcElement||t.target;while(s!==w){if(w.tagName.toLowerCase()==="img"){var r=w.parentNode.parentNode;x=r.getAttribute("data-account");v=r.getAttribute("data-scene");a(x,v);return}else{if(w.tagName.toLowerCase()==="li"){x=w.getAttribute("data-account");v=w.getAttribute("data-scene");c(x,v);return}}w=w.parentNode}});this.update(d);if(!!e){this.inject(e)}};j.prototype.inject=function(b){var a=i.getNode(b);a.innerHTML="";a.appendChild(this._body)};j.prototype.update=function(p){var e="",o,d,r,b=p.sessions;if(b.length===0){e+='<p class="empty">暂无最近联系人哦</p>'}else{for(o=0;o<b.length;o++){r=this.provider(b[o],"session");d=['<li class="panel_item '+(r.crtSession===r.target?"active":"")+'" data-scene="'+r.scene+'" data-account="'+r.account+'">','<div class="panel_avatar"><img class="panel_image" src="'+r.avatar+'"/></div>','<div class="panel_text">','<p class="panel_multi-row">','<span class="panel_nick">'+r.nick+"</span>",'<b class="panel_time">'+r.time+"</b>","</p>",'<p class="panel_multi-row">','<span class="panel_lastMsg">'+r.text+"</span>",r.unread?'<b class="panel_count '+(r.unread>0?"":"hide")+'">'+r.unread+"</b>":"","</p>","</div>","</li>"].join("");e+=d;if(r.unread>0){var a=$("#chatWindow",window.parent.document);if(a.hasClass("windowHidden")){var q=$("#eidpChat",window.parent.document);var c=q.find(".badge-count");if(!c.length){c=$("<span class='badge badge-count'></span>");q.append(c);c.text(r.unread)}else{c.text(r.unread)}}}}}this._body.innerHTML=e};j.prototype.destory=function(){};g.exports=j},function(h,e){var f={getNode:function(a,b){if(this.isString(a)){b=b||document;return b.querySelector(a)}else{if(this.isElement(a)){return a}else{console.error("输入参数必须为node||String")}}},getNodes:function(a){return document.querySelectorAll(a)},isString:function(a){return typeof(a)==="string"},isElement:function(a){return !!(a&&a.nodeType===1)},isArray:Array.isArray||function(a){return Object.prototype.toString.call(a)==="[object Array]"},addEvent:function(a,b,c){if(window.addEventListener){a.addEventListener(b,c,false)}else{a.attachEvent("on"+b,c)}},hasClass:function(a,b){b=b||"";if(b.replace(/\s/g,"").length===0){return false}return new RegExp(" "+b+" ").test(" "+a.className+" ")},addClass:function(a,b){if(!a){return}if(!this.hasClass(a,b)){a.className+=" "+b}},removeClass:function(a,b){if(!a){return}if(this.hasClass(a,b)){var c=" "+a.className.replace(/[\t\r\n]/g,"")+" ";while(c.indexOf(" "+b+" ")>=0){c=c.replace(" "+b+" "," ")}a.className=c.replace(/^\s+|\s+$/g,"")}},safeHtml:(function(){var b=/<br\/?>$/,a={r:/<|>|\&|\r|\n|\s|\'|\"/g,"<":"&lt;",">":"&gt;","&":"&amp;"," ":"&nbsp;",'"':"&quot;","'":"&#39;","\n":"<br/>","\r":""};return function(c){c=g(a,c);return c.replace(b,"<br/><br/>")}})()};var g=function(a,b){b=""+b;if(!a||!b){return b||""}return b.replace(a.r,function(d){var c=a[!a.i?d.toLowerCase():d];return c!=null?c:d})};h.exports=f},function(i,j,g){var l=g(2);var k;var h=function(b){var e=b.parent,d=b.data,c=b.onclickitem||function(f,n){console.log("account:"+f+"---type:"+n)},a=b.onclickavatar||function(f,n){console.log("account:"+f+"---type:"+n)};k=b.account;this.provider=b.infoprovider;this._body=document.createElement("ul");this._body.className=b.clazz||"m-panel j-friend";l.addEvent(this._body,"click",function(t){var f=this,s=t||window.event,w,u,v=s.srcElement||s.target;while(f!==v){if(v.tagName.toLowerCase()==="img"){var x=v.parentNode.parentNode;w=x.getAttribute("data-account");u=x.getAttribute("data-scene");a(w,u);return}else{if(v.tagName.toLowerCase()==="li"){w=v.getAttribute("data-account");u=v.getAttribute("data-scene");c(w,u);return}}v=v.parentNode}});this.update(d);if(!!e){this.inject(e)}};h.prototype.inject=function(b){var a=l.getNode(b);a.innerHTML="";a.appendChild(this._body)};h.prototype.update=function(e){var b="",a=e.friends,d;for(var c=0;c<a.length;c++){d=this.provider(a[c],"friend");if(a[c].account!==k){b+=['<li class="panel_item '+(d.crtSession===d.target?"active":"")+'" data-scene="p2p" data-account="'+d.account+'">','<div class="panel_avatar"><img class="panel_image" src="'+d.avatar+'"/></div>','<div class="panel_text">','<p class="panel_multi-row">'+d.nick+"</p>",'<p class="panel_multi-row">'+d.dept+"</p>","</div>","</li>"].join("")}}this._body.innerHTML=b};h.prototype.destory=function(){};i.exports=h},function(f,h,j){var i=j(2);var g=function(l){var d=this,e=l.parent,b=l.data,c=l.onclickitem||function(k,n){console.log("account:"+k+"---type:"+n)},a=l.onclickavatar||function(k,n){console.log("account:"+k+"---type:"+n)};this._body=document.createElement("ul");this._body.className=(l.clazz||"m-panel")+" j-team";this.provider=l.infoprovider;i.addEvent(this._body,"click",function(k){var w=this,u=k||window.event,x,s,t=u.srcElement||u.target;while(w!==t){if(t.tagName.toLowerCase()==="img"){var v=t.parentNode.parentNode;x=v.getAttribute("data-account");s=v.getAttribute("data-type");a(x,s);return}else{if(t.tagName.toLowerCase()==="li"){x=t.getAttribute("data-account");s=t.getAttribute("data-type");i.removeClass(i.getNode(".j-team li.active",d._body),"active");i.addClass(t,"active");c(x,s);return}}t=t.parentNode}});this.update(b);if(!!e){this.inject(e)}};g.prototype.inject=function(b){var a=i.getNode(b);a.innerHTML="";a.appendChild(this._body)};g.prototype.update=function(l){var r='<div class="panel_team"><div class="panel_team-title">讨论组</div><ul class="j-normalTeam">',s='<div class=" panel_team"><div class="panel_team-title">高级群</div><ul class="j-advanceTeam">',b=false,c=false,e="",a,d=l.teams;if(d&&d.length>0){for(var q=0,t=d.length;q<t;++q){a=this.provider(d[q],"team");if(a.type==="normal"){b=true;r+=['<li class="panel_item '+(a.crtSession===a.target?"active":"")+'" data-gtype="normal" data-type="team" data-account="'+a.teamId+'">','<div class="panel_avatar"><img class="panel_image" src="'+a.avatar+'"/></div>','<div class="panel_text">','<p class="panel_single-row">'+a.nick+"</p>","</div>","</li>"].join("")}else{if(a.type==="advanced"){c=true;s+=['<li class="panel_item '+(a.crtSession===a.target?"active":"")+'" data-gtype="advanced" data-type="team" data-account="'+a.teamId+'">','<div class="panel_avatar"><img class="panel_image" src="'+a.avatar+'"/></div>','<div class="panel_text">','<p class="panel_single-row">'+a.nick+"</p>","</div>","</li>"].join("")}}}r+="</ul></div>";s+="</ul></div>";if(b&&c){e=s+r}else{if(b&&!c){e=r}else{if(!b&&c){e=s}else{e="<p>暂时还没有群哦</p>"}}}}else{e="<p>暂时还没有群哦</p>"}this._body.innerHTML=e};g.prototype.destory=function(){};f.exports=g}])});
...
var appUI={buildChatContentUI:function(k,o){var j="",l=o.getMsgs(k);if(l.length===0){j='<div class="no-msg tc"><span class="radius5px">暂无消息</span></div>'}else{for(var n=0,i=l.length;n<i;++n){var m=l[n],p=o.getUserById(m.from);if(n==0){j+=this.makeTimeTag(transTime(m.time))}else{if(m.time-l[n-1].time>5*60*1000){j+=this.makeTimeTag(transTime(m.time))}}j+=this.makeChatContent(m,p)}}return j},updateChatContentUI:function(j,g){var f=$("#j-chatContent .item").last(),i="",h=g.getUserById(j.from);if(f.length==0){i+=this.makeTimeTag(transTime(j.time))}else{if(j.time-parseInt(f.attr("data-time"))>5*60*1000){i+=this.makeTimeTag(transTime(j.time))}}i+=this.makeChatContent(j,h);return i},makeChatContent:function(l,p){var j;if(l.attach&&l.attach.type){var i=transNotification(l);j='<p class="u-notice tc item" data-time="'+l.time+'" data-id="'+l.idClient+'" data-idServer="'+l.idServer+'"><span class="radius5px">'+i+"</span></p>"}else{var m=l.type,k=l.from,o=p.avatar,n=l.scene==="team"&&k!==userUID,j=['<div data-time="'+l.time+'" data-id="'+l.idClient+'" data-idServer="'+l.idServer+'" class="item item-'+buildSender(l)+'">','<img class="img j-img" src="'+getAvatar(o,p.gender)+'" data-account="'+k+'"/>',n?'<p class="nick">'+getNick(k)+"</p>":"",'<div class="msg msg-text">','<div class="box">','<div class="cnt">',getMessage(l),"</div>","</div>","</div>",l.status==="fail"?'<span class="error"><i class="im-icon imicon-error"></i>发送失败</span>':"","</div>"].join("")}return j},buildCloudMsgUI:function(g,j){var k="",l=g.length,h;for(var i=l-1;i>=0;--i){message=g[i];if(i==(l-1)){k+=this.makeTimeTag(transTime(message.time))}else{if(message.time-g[i+1].time>5*60*1000){k+=this.makeTimeTag(transTime(message.time))}}k+=this.makeChatContent(message,j.getUserById(message.from))}return k},buildTeamMemberList:function(b){return['<li data-icon="'+b.avatar+'" data-uid="'+b.account+'" data-account="'+b.nick+'">','<i class="im-icon imicon-radio"></i>','<img src="'+getAvatar(b.avatar,b.gender)+'">','<span class="name">'+getNick(b.account)+"</span>","</li>"].join("")},buildBlacklist:function(j,h){var f="";if(j.length===0){return""}for(var g=0;g<j.length;g++){var i=h.getUserById(j[g].account);f+=['<li class="items f-cb">','<img src="'+getAvatar(i.avatar,i.gender)+'" class="head">','<span class="nick">'+i.nick+"</span>",'<button class="btn radius4px btn-ok j-rm" data-id="'+i.account+'">解除</button>',"</li>"].join("")}return f},buildSysMsgs:function(k,j){var o="",m,i,l,n;if(k.length===0){return""}for(var p=0;p<k.length;p++){m=k[p];if(m.category=="team"){i=m.attach?m.attach.team:j.getTeamMapById(m.to);if(!i){continue}if(m.type==="teamInvite"){n=getNick(m.from)+"邀请你入群";if(m.state==="init"){l='<a class="j-apply">同意</a><a class="j-reject">拒绝</a>'}else{if(m.state==="rejected"){l="已拒绝"}else{if(m.state==="passed"){l="已同意"}else{l="已失效"}}}}else{if(m.type==="applyTeam"){n=getNick(m.from)+"申请加入群";if(m.state==="init"){l='<a class="j-apply">同意</a><a class="j-reject">拒绝</a>'}else{if(m.state==="rejected"){l="已拒绝"}else{if(m.state==="passed"){l="已同意"}else{l="已失效"}}}}else{if(m.type==="rejectTeamApply"||m.type==="rejectTeamInvite"){n=getNick(m.from)+"拒绝了你的入群邀请";l="已拒绝"}else{n="未知";l=""}}}o+=['<div class="item">','<img src="images/advanced.png">','<div class="text">',"<p><span>"+(i?i.name:m.to)+'</span><b class="time">'+transTime2(m.time)+"</b></p>",'<p><span class="first-msg">'+n+'</span><b class="action" data-type="'+m.type+'" data-idServer="'+m.idServer+'" data-id="'+i.teamId+'" data-from="'+m.from+'">'+l+"</b></p>","</div>","</div>"].join("")}else{if(m.category=="friend"){}}}return o},buildCustomSysMsgs:function(t,x){var r="",q,o,s,n,w,p;if(t.length===0){return r}t=t.sort(function(b,a){return a.time-b.time});for(var u=0;u<t.length;u++){s=t[u].scene;q=JSON.parse(t[u].content).content;o=t[u].from;n=t[u].to;if(s==="p2p"){w=getNick(o);var v=x.getUserById(o);if(v){p=getAvatar(v.avatar,v.gender)}else{p=getAvatar("","")}}else{var i=x.getTeamById(n+"");if(i){w=i.name+"-->"+getNick(o);p="images/"+i.type+".png"}else{w=n+"-->"+getNick(o);p="images/normal.png"}}r+=['<div class="item">','<img src="'+p+'">','<div class="text">',"<p><span>"+w+'</span><b class="time">'+transTime2(t[u].time)+"</b></p>",'<p><span class="first-msg">'+q+"</span></p>","</div>","</div>"].join("")}return r},makeTimeTag:function(b){return'<p class="u-msgTime">- - - - -&nbsp;'+b+"&nbsp;- -- - -</p>"}};
var myTeam={init:function(d,c){this.cache=d;this.sdk=c;this.initNode();this.addEvent()},initNode:function(){this.$teamPanel=$("#chatList-3");this.$teamInfo=$("#btn-teamInfo");this.$rightPanel=$("#rightContent");this.$mask=$("#j-mask");this.$teamInfoContainer=$("#teamInfo");this.$createTeamContainer=$("#j-createTeamContainer");this.$searchBox=$("#searchTeamBox")},addEvent:function(){this.$teamPanel.delegate("#createTeam","click",this.showUserList.bind(this,0,0));this.$teamPanel.delegate("#createAdvanceTeam","click",this.showUserList.bind(this,0,1));this.$teamPanel.delegate("#searchAdvanceTeam","click",this.showSearch.bind(this,0,1));this.$teamInfoContainer.delegate(".j-backBtn","click",this.closeTeamPanel.bind(this));this.$teamInfoContainer.delegate("#j-userList .first","click",this.showUserList.bind(this,1));this.$teamInfoContainer.delegate("#j-userList .hover","click",this.removeUser.bind(this));this.$teamInfoContainer.delegate("#j-teamName","click",this.editTeamName);this.$teamInfoContainer.delegate("#j-desc","click",this.editDesc);this.$teamInfoContainer.delegate("#exitTeam","click",this.exitTeam.bind(this));this.$teamInfoContainer.delegate(".j-dismissTeam","click",this.dismissTeam.bind(this));this.$teamInfoContainer.delegate("#j-nameInput","blur",this.saveTeamName.bind(this));this.$teamInfoContainer.delegate("#j-descInput","blur",this.saveTeamDesc.bind(this));this.$teamInfoContainer.delegate(".j-joinMode","change",this.setJoinMode);this.$createTeamContainer.delegate(".user-list li","click",this.checkedUser);this.$createTeamContainer.delegate(".icon-close","click",this.closeDialog.bind(this));this.$createTeamContainer.on("click","#j-btnAdd",this.createTeam.bind(this));this.$createTeamContainer.on("click","#j-btnCancel",this.closeDialog.bind(this));this.$teamInfo.on("click",this.showTeamInfo.bind(this,this.$teamInfo));this.$searchBox.delegate(".j-close","click",this.hideSearch.bind(this));this.$searchBox.delegate(".j-search","click",this.doSearch.bind(this));this.$searchBox.delegate(".j-back","click",this.resetSearch.bind(this));this.$searchBox.delegate(".j-chat","click",this.doChat.bind(this));this.$searchBox.delegate(".j-add","click",this.doAdd.bind(this))},closeDialog:function(){this.$createTeamContainer.addClass("hide");this.$mask.addClass("hide")},editTeamName:function(){var b=$(this);if(b.hasClass("owner")){b.parent(".wrap").addClass("active");b.parent(".wrap").find("input").focus()}},editDesc:function(){var b=$(this);if(b.hasClass("owner")){b.parent(".wrap").addClass("active");b.parent(".wrap").find("input").focus()}},saveTeamName:function(){var j=$("#j-nameInput"),h=$("#j-teams"),i=j.val().trim(),g=this,f=this.$teamInfo.data("team-id");if(i.length>0){this.sdk.updateTeam({teamId:f,name:i,done:function(a,c){if(!a){var b=c.name;$("#j-teamName .name").text(b);j.val("").parents(".wrap").removeClass("active")}else{$.core.showErrorInfo("群名称修改失败")}}})}else{j.parents(".wrap").removeClass("active")}},saveTeamDesc:function(){var j=$("#j-descInput"),h=$("#j-teams"),i=j.val().trim(),g=this,f=this.$teamInfo.data("team-id");if(i.length>0){this.sdk.updateTeam({teamId:f,intro:i,done:function(a,c){if(!a){var b=c.intro;$("#j-desc .name").text(b);j.val("").parents(".wrap").removeClass("active")}else{$.core.showErrorInfo("修改群介绍失败")}}})}else{j.parents(".wrap").removeClass("active")}},createTeam:function(){var j=[],h=[],g=$("#j-addedUserList ul li"),i=this.addTag&&this.$teamInfo.data("team-id"),k=this;if(g.length===0){this.$createTeamContainer.addClass("hide");this.$teamInfoContainer.addClass("hide");this.$mask.addClass("hide");return}$("#j-addedUserList ul").html("");$("#j-addedUserNum").text("0");g.map(function(){j.push($(this).attr("data-uid"));h.push($(this).attr("data-account"))});if(!!i){this.sdk.addTeamMembers({teamId:i,accounts:j,ps:"",done:function(b,a){if(b){$.core.showErrorInfo("添加成员失败!原因："+b.message)}}})}else{var l=this.getUserById(userUID).nick;h=[l].concat(h).join("、").slice(0,20);if(k.teamType===0){this.sdk.createTeam({type:"normal",name:h+"等人",accounts:j,done:function(b,a){if(!b){k.cache.addTeam(a.team);im.buildTeams();$('#j-loadTeams .j-normalTeam li[data-account="'+a.team.teamId+'"]').click()}else{$.core.showErrorInfo("创建群组失败!原因："+b.message)}}})}else{this.sdk.createTeam({type:"advanced",name:h+"等人",accounts:j,joinMode:"needVerify",done:function(b,a){if(!b){k.cache.addTeam(a.team);im.buildTeams();$('#j-loadTeams .j-advanceTeam li[data-account="'+a.team.teamId+'"]').click()}else{$.core.showErrorInfo(b.message)}}})}}this.$createTeamContainer.addClass("hide");this.$teamInfoContainer.addClass("hide");this.$rightPanel.removeClass("hide");this.$mask.addClass("hide")},getUsers:function(l,h){var j=[];for(var i=0,k=l.length;i<k;++i){var a=this.getUserById(l[i]);if(h===0){a.type=i>0?"normal":"owner"}else{a.type="normal"}j.push(a)}return j},showUserList:function(p,i){var r=this;this.teamType=i;$("#j-devices").addClass("hide");r.$createTeamContainer.removeClass("hide");r.$mask.removeClass("hide");var o=$("#j-userList .first"),n=$("#j-addUserList ul"),q=r.cache.getFriendslistOnShow(),s="",t="",v=[];for(var u=0,l=q.length;u<l;++u){if(q[u].uid!==userUID){s+=appUI.buildTeamMemberList(q[u])}}n.html(s);if(p!==0){t=o.data("team-id");r.sdk.getTeamMembers({teamId:t,done:function(d,a){v=d?getMembersById(t):a.members;for(var b=0,e=v.length;b<e;++b){var c=v[b].account||v[b].uid;n.find('[data-uid="'+c+'"] i').addClass("cur2")}}});r.addTag=true}else{r.addTag=false}},showTeamInfo:function(F){var A=$(F),v=this;this.$rightPanel.addClass("hide");this.$teamInfoContainer.removeClass("hide");var H=A.data("team-id"),D=v.cache.getTeamById(H);if(!D){return}var u=D.type,x=D.name,t=D.owner,B=D.intro||"",E=D.joinMode||"",y=[],w="",o=$("#j-userList"),z=v.$teamInfoContainer.find(".j-teamId"),G=$("#j-teamName"),s=$("#j-desc");if(u==="advanced"){v.$teamInfoContainer.find(".j-advanced").removeClass("hide");s.find(".name").text(B);if(t===userUID){v.$teamInfoContainer.find(".j-joinMode[value="+E+"]").attr("checked","checked")}else{v.$teamInfoContainer.find(".j-joinMode").attr("disabled","disabled").filter("[value="+E+"]").attr("checked","checked")}}if(userUID===t&&u==="advanced"){v.$teamInfoContainer.find(".j-dismissTeam").removeClass("hide");v.$teamInfoContainer.find("#exitTeam").addClass("hide")}else{if(u==="normal"){v.$teamInfoContainer.find("#exitTeam").text("退出讨论组")}}if(userUID!=t){v.$teamInfoContainer.find(".imicon-write").addClass("hide")}y=v.cache.getTeamMembers(H);G.find(".name").text(x);z.text(H);var C=function(d){v.sortMembers(d);var a=[];var b=function(){if(u==="normal"||userUID===t){w+='<li class="first add-item tc radius-circle" data-team-type="'+u+'" data-team-id="'+H+'"><i class="icon icon-plus"></i><p></p></li>'}for(var f=0,h=d.length;f<h;++f){var k=d[f],i=k.account||k.uid,g=getNick(i);var j;var e=v.getUserById(i);if(e){j=getAvatar(e.avatar,e.gender)}else{j=getAvatar("","")}w+='<li data-uid="'+i+'"><a href="javascript:;"><img src="'+j+'"/>';if(k.type==="owner"){w+='<i class="im-icon radius-circle imicon-user"></i>'}else{w+='<span class="hover" data-nick="'+g+'" data-team-name="'+x+'" data-uid="'+i+'" data-team-id="'+H+'">移除</span>'}w+='</a><p class="text">'+g+"</p></li>"}o.html(w);if(u==="normal"||t===userUID){G.addClass("owner");s.addClass("owner")}if(t===userUID){o.addClass("owner")}};for(var c=0;c<d.length;c++){if(!v.getUserById(d[c].account)){a.push({uid:d[c].account})}}if(a.length>0){im.mysdk.getUsers(a,function(e){for(var f=0;f<e.list.length;f++){v.cache.updatePersonlist(e.list[f])}b()})}else{b()}};v.sdk.getTeamMembers({teamId:H,done:function(b,a){if(!b){y=a.members;C(y)}else{console&&console.error("获取群成员失败")}}});v.$teamInfoContainer.removeClass("hide")},getUserById:function(b){return this.cache.getUserById(b)},getTeamById:function(b){return this.cache.getTeamById(b)},getMembersById:function(b){return this.cache.getMembersById(b)},sortMembers:function(b){if(!b||!b.length){return}b.sort(function(a,d){if(a.type>d.type){return -1}else{return 1}})},checkedUser:function(t){var p=$(this),m=p.find("i"),o=$("#j-addedUserNum"),s=$("#j-addedUserList ul"),r=p.attr("data-uid"),l=p.data("account"),n=p.data("icon"),k=o.text();if(!m.hasClass("cur2")){m.toggleClass("cur");var q='<li data-uid="'+r+'" data-account="'+l+'" data-icon="'+n+'"><img src="'+getAvatar(n)+'" width="48" height="48"/><p class="name">'+l+"</p></li>";if(m.hasClass("cur")){s.append(q);k++}else{s.find("[data-uid="+r+"]").remove();k--}o.text(k)}},removeUser:function(k){var l=k||window.event,h=$(l.target),m=this,n=h.attr("data-uid"),j=h.data("team-id"),i=$("#j-userList .first").data("team-type");this.sdk.removeTeamMembers({teamId:j,accounts:[n],done:function(b,a){if(b){$.core.showErrorInfo("移除成员失败!原因："+b.message)}else{h.parents("li").remove()}}})},buildTeamName:function(j){var g=[];for(var h=0,l=j.length;h<l;++h){var i=j[h].uid||j[h].account,k=this.getUserById(i);g.push(k.name)}return g.join("、")+"等人"},setJoinMode:function(){var d=$(this).val(),c=myTeam.$teamInfo.data("team-id");myTeam.sdk.updateTeam({teamId:c,joinMode:d,done:function(b,a){if(!b){}else{$.core.showErrorInfo(b.message)}}})},exitTeam:function(){var d=this,c=this.$teamInfo.data("team-id");this.sdk.leaveTeam({teamId:c,done:function(b,a){if(!b){$("#j-chatEditor").data({to:""});d.$teamInfoContainer.addClass("hide");d.$rightPanel.addClass("hide");removeChatVernier(c)}else{$.core.showErrorInfo(b.message)}}})},dismissTeam:function(){var d=this,c=this.$teamInfo.data("team-id");this.sdk.dismissTeam({teamId:c,done:function(b,a){if(!b){$("#j-chatEditor").data({to:""});d.$teamInfoContainer.addClass("hide");d.$rightPanel.addClass("hide");removeChatVernier(c)}else{$.core.showErrorInfo(b.message)}}})},closeTeamPanel:function(){this.$teamInfoContainer.addClass("hide");this.$mask.addClass("hide");this.$rightPanel.removeClass("hide")},showSearch:function(){this.searchData=null;this.$searchBox.removeClass("hide");this.$mask.removeClass("hide");this.$searchBox.find(".j-account").focus()},hideSearch:function(){this.resetSearch();this.$searchBox.addClass("hide");this.$mask.addClass("hide")},doSearch:function(){var b=$.trim(this.$searchBox.find(".j-account").val());if(/^\d+$/.test(b)){this.sdk.getTeam(b,this.cbDoSearch.bind(this))}else{$.core.showErrorInfo("输入有误（群ID必须是数字）")}},cbDoSearch:function(e,h){if(e){$.core.showErrorInfo(e.message)}else{if(h.type==="normal"||h.valid===false){$.core.showErrorInfo("群不存在");return}var f=this.$searchBox.find(".info");var g=h.teamId;f.find(".j-name").html(h.name);f.find(".j-teamId").html(h.teamId);if(this.cache.hasTeam(g)){this.$searchBox.addClass("inTeam")}else{this.$searchBox.addClass("notInTeam")}}},resetSearch:function(){this.$searchBox.attr("class","m-dialog");this.$searchBox.find(".j-account").val("")},doChat:function(){var b=$.trim(this.$searchBox.find(".j-account").val());im.openChatBox(b,"team");this.hideSearch()},doAdd:function(){var b=$.trim(this.$searchBox.find(".j-account").val());this.sdk.applyTeam(b)},};
/**
 * 群通知处理
 */
...
var emojiInited=false;var step=0;var page=1;var srcTitle=window.parent.document.title;var im={init:function(f,d,e){$.core.showProgress();this.initNode();this.cache=new Cache();this.mysdk=new SDKBridge(this,this.cache,f,d,e);this.buildContacts();myTeam.init(this.cache,this.mysdk);this.addEvent();notification.init(this.cache,this.mysdk);$.core.hidenProgress()},initNode:function(){this.$userName=$("#j-userName");this.$userPic=$("#j-userPic");this.$conversations=$("#j-conversations");this.$loadConversations=$("#j-loadConversations");this.$loadContacts=$("#j-loadContacts");this.$loadTeams=$("#j-loadTeams");this.$chatContent=$("#j-chatContent");this.$sendBtn=$("#sendMessage");this.$contacts=$("#j-contacts");this.$teamInfoContainer=$("#teamInfo");this.$cloudMsgContainer=$("#cloudMsg");this.$rightPanel=$("#rightContent");this.$nickName=$("#j-nickName");this.$headImg=$("#headImg");this.$teamInfoBtn=$("#btn-teamInfo");this.$cloudMsgBtn=$("#btn-cloudMsg");this.$messageText=$("#txtMessage");this.$chatEditor=$("#sendMsgContent");this.$showEmoji=$("#sendEmoji");this.$myInfo=$("#myInfo");this.$modifyAvatar=$("#modifyAvatar");this.$personCard=$("#personCard");this.$chooseFileBtn=$("#chooseFile");this.$fileInput=$("#uploadFiles");this.$mask=$("#j-mask");this.$shareList=$("#shareList");this.$shareContent=$("#shareContent");this.$sendShareMsg=$("#sendShare");this.$cancelShareMsg=$("#cancelShare");this.$notice=$("#sysNoticePanel")},initUI:function(){this.buildConversations()},addEvent:function(){this.$cloudMsgBtn.on("click",this.showCloudMsg.bind(this,this.$cloudMsgBtn));$("#cloudMsg").delegate(".j-backBtn","click",this.closeCloudMsgContainer.bind(this));$("#cloudMsg").delegate(".j-loadMore","click",this.loadMoreCloudMsg.bind(this));this.$sendBtn.on("click",this.sendTextMessage.bind(this));this.$messageText.on("keydown",this.inputMessage);this.$showEmoji.on("click",this.showEmoji.bind(this));this.$chooseFileBtn.on("click",this.chooseFile.bind(this));this.$fileInput.on("change",this.sendFile.bind(this));this.$personCard.delegate(".j-close","click",this.hideInfoBox.bind(this));$("#headImg").on("click",this.showInfo2.bind(this));this.$shareList.delegate(".j-close","click",this.hideSendMessage.bind(this));this.$sendShareMsg.on("click",this.sendShareMessage.bind(this));this.$cancelShareMsg.on("click",this.cancelShareMessage.bind(this));this.$cloudMsgContainer.delegate(".j-mbox","click",this.playAudio);this.$chatContent.delegate(".j-mbox","click",this.playAudio);this.$cloudMsgContainer.delegate(".convert","click",this.convertAudio);this.$chatContent.delegate(".convert","click",this.convertAudio);this.$cloudMsgContainer.delegate(".location","click",this.showLocation);this.$chatContent.delegate(".location","click",this.showLocation);$("#systemNotice").on("click",this.showSystemNotice.bind(this))},initEmoji:function(){if(!emojiInited){var c=this,d={emojiList:emojiList,pinupList:pinupList,width:500,height:300,imgpath:"/view/static/comp/im/images/",callback:function(a){c.cbShowEmoji(a)}};emojiInited=true;this.$emNode=new CEmojiEngine($("#emojiTag")[0],d)}},selectTabs:function(b){if($(b.item).attr("id")=="teams"||$(b.item).attr("id")=="teamInfos"){this.buildTeams()}},showMe:function(){var b=this.cache.getUserById(this.userUID);this.$userName.text(b.nick)},initInfo:function(i,f){this.lockPerson=true;this.lockTeam=true;var g=Object.keys(i),h=[];for(var j=f.length-1;j>=0;j--){if(!this.cache.hasTeam(f[j])){h.push(f[j])}}if(h.length>0){this.mysdk.getLocalTeams(h,this.cbInitLocakTeamInfo.bind(this))}else{this.lockTeam=false}this.mysdk.getUsers(g,this.cbInitInfo.bind(this))},buildContacts:function(){this.cache.setFriends(emps);this.cache.setPersonlist(emps);var f={friends:emps,account:userUID};if(!this.friend){var d={data:f,onclickavatar:this.showInfo.bind(this),onclickitem:this.openChatBox.bind(this),infoprovider:this.infoProvider.bind(this)};this.friends=new NIMUIKit.FriendList(d);this.friends.inject(this.$contacts.get(0));var e={data:f,onclickavatar:this.showSendMessage.bind(this),onclickitem:this.showSendMessage.bind(this),infoprovider:this.infoProvider.bind(this)};this.shareEmps=new NIMUIKit.FriendList(e);this.shareEmps.inject($("#empInfos").get(0))}else{this.friends.update(f);this.shareEmps.update(f)}},getNick:function(b){return getNick(b,this.cache)},cbInitInfo:function(d,c){if(!d){this.cache.setPersonlist(c);this.lockPerson=false;if(this.lockTeam===false){this.initUI()}}else{$.core.showErrorMsg("获取用户信息失败","系统提示")}},cbInitLocakTeamInfo:function(d,c){if(!d){this.cache.addTeamMap(c.teams);this.lockTeam=false;if(this.lockPerson===false){this.initUI()}}else{$.core.showErrorMsg("获取本地群组失败","系统提示")}},buildConversations:function(){var c={sessions:this.cache.getSessions()};if(!this.sessions){var d={data:c,onclickavatar:this.showInfo.bind(this),onclickitem:this.openChatBox.bind(this),infoprovider:this.infoProvider.bind(this),};this.sessions=new NIMUIKit.SessionList(d);this.sessions.inject(this.$conversations.get(0))}else{this.sessions.update(c)}},buildTeams:function(){var f={teams:this.cache.getTeamlist()};if(!this.teams){var d={data:f,infoprovider:this.infoProvider.bind(this),onclickavatar:this.clickTeamAvatar.bind(this),onclickitem:this.openChatBox.bind(this)};this.teams=new NIMUIKit.TeamList(d);this.teams.inject($("#j-teams").get(0));var e={data:f,infoprovider:this.infoProvider.bind(this),onclickavatar:this.showSendMessage.bind(this),onclickitem:this.showSendMessage.bind(this)};this.shareTeams=new NIMUIKit.TeamList(e);this.shareTeams.inject($("#teamInfos").get(0))}else{this.teams.update(f);this.shareTeams.update(f)}},clickTeamAvatar:function(c,d){$("#j-teams").find("li.active").removeClass("active");this.openChatBox(c,d)},showSendMessage:function(d,c){this.openChatBox(d,c);this.$shareList.addClass("hide");$("#shareMessage").removeClass("hide");this.$shareContent.data({to:d,scene:c,type:$("#msgType").val(),relTypeName:$("#relTypeName").val(),relType:$("#relType").val(),relNo:$("#relNo").val()})},hideSendMessage:function(){this.$shareList.addClass("hide");this.$mask.addClass("hide")},sendShareMessage:function(){var n=this.$shareContent.data("scene");var h=this.$shareContent.data("to");var i=this.$shareContent.data("relTypeName");var k=this.$shareContent.data("relType");var l=this.$shareContent.data("relNo");var j=this.$shareContent.data("type");var m={relTypeName:i,relType:k,relNo:l,type:kendo.parseInt(j)};this.mysdk.sendCustomMessage(n,h,m,this.sendMsgDone.bind(this));$("#shareMessage").addClass("hide");this.$mask.addClass("hide")},cancelShareMessage:function(){$("#shareMessage").addClass("hide");this.$shareList.removeClass("hide")},openChatBox:function(e,f){var d;this.mysdk.setCurrSession(f,e);this.crtSession=f+"-"+e;if(f=="p2p"){d=this.cache.getUserById(e)}else{d=this.cache.getTeamById(e)}this.$teamInfoContainer.addClass("hide");this.$cloudMsgContainer.addClass("hide");this.$rightPanel.find(".u-chat-notice").addClass("hide");this.$rightPanel.find(".chat-mask").addClass("hide");this.$notice.addClass("hide");if(f==="p2p"){if(d.account==userUID){this.$nickName.text("我的手机");this.$headImg.attr("src",ctx+"/view/static/comp/im/images/myPhone.png")}else{this.$nickName.text(this.getNick(e));this.$headImg.attr("src",getAvatar(d.avatar,d.gender))}}else{if(d){this.$headImg.attr("src",ctx+"/view/static/comp/im/images/"+d.type+".png");this.$nickName.text(d.name)}else{this.$rightPanel.find(".u-chat-notice").removeClass("hide");this.$rightPanel.find(".chat-mask").removeClass("hide");this.$headImg.attr("src",ctx+"/view/static/comp/im/images/normal.png");this.$nickName.text(e)}}this.$rightPanel.removeClass("hide");this.$messageText.val("");if(f==="p2p"){this.$teamInfoBtn.addClass("hide").data({teamId:"",gtype:""})}else{this.$teamInfoBtn.removeClass("hide").data({teamId:e,gtype:d?d.type:"normal"})}this.$chatEditor.data({to:e,type:f});this.getHistoryMsgs(f,e)},infoProvider:function(k,m){var i={};switch(m){case"session":var p=k.lastMsg,j=p.scene;i.scene=p.scene;i.account=p.target;i.target=p.scene+"-"+p.target;i.time=transTime2(p.time);i.crtSession=this.crtSession;i.unread=k.unread>99?"99+":k.unread;i.text=buildSessionMsg(p);if(j==="p2p"){if(p.target===userUID){i.nick="我的手机";i.avatar=ctx+"/view/static/comp/im/images/myPhone.png"}else{var o=this.cache.getUserById(p.target);i.nick=this.getNick(p.target);i.avatar=getAvatar(o.avatar,o.gender)}}else{var n=this.cache.getTeamById(p.target);if(n){i.nick=n.name;i.avatar=ctx+"/view/static/comp/im/images/"+n.type+".png"}else{i.nick=p.target;i.avatar=ctx+"/view/static/comp/im/images/normal.png"}}break;case"friend":i.target="p2p-"+k.account;i.account=k.account;i.nick=this.getNick(i.account);i.avatar=getAvatar(k.avatar,k.gender);var l=JSON.parse(k.custom);if(l){i.dept=l.department}i.crtSession=this.crtSession;break;case"team":i.type=k.type;i.nick=k.name;i.target="team-"+k.teamId;i.teamId=k.teamId;i.avatar=i.type==="normal"?ctx+"/view/static/comp/im/images/normal.png":ctx+"/view/static/comp/im/images/advanced.png";i.crtSession=this.crtSession;break}return i},getHistoryMsgs:function(h,i){var m=h+"-"+i;var n=this.cache.findSession(m);var j=this.cache.getMsgs(m);if(!!n){if(n.unread>=j.length){var k=(j.length>0)?j[0].idClient:false;this.mysdk.getLocalMsgs(h,i,k,this.getLocalMsgsDone.bind(this));return}}var l=appUI.buildChatContentUI(m,this.cache);this.$chatContent.html(l);this.$chatContent.scrollTop(9999)},getLocalMsgsDone:function(g,f){if(!g){this.cache.addMsgsByReverse(f.msgs);var e=f.scene+"-"+f.to;var h=appUI.buildChatContentUI(e,this.cache);this.$chatContent.html(h);this.$chatContent.scrollTop(9999)}else{$.core.showErrorInfo("获取历史消息失败")}},showInfo:function(f,d){if(d=="p2p"){var e=im.cache.getUserById(f);this.showInfoBox(e)}},showInfo2:function(){if($("#sendMsgContent").data("type")=="p2p"){var c=$("#sendMsgContent").data("to");var d=im.cache.getUserById(c);this.showInfoBox(d)}},showInfoBox:function(m){if(m.account===userUID){this.showMyInfo();return}var l=this.cache.isFriend(m.account);var p=this.cache.inMutelist(m.account);var n=this.cache.inBlacklist(m.account);var o=this.$personCard.data({account:m.account,inMutelist:p?true:false,inBlacklist:n?true:false});o.find(".u-icon").attr("src",getAvatar(m.avatar,m.gender));o.find(".j-nick").text(getNick(m.account));var i="";if(m.gender&&m.gender!=="unknown"){i=ctx+"/view/static/comp/im/images/"+m.gender+".png"}else{o.find(".j-gender").addClass("hide")}o.find(".j-gender").attr("src",i);o.find(".j-username").text("帐号："+m.account);o.find(".j-birth").text(m.birth===undefined?"--":m.birth);o.find(".j-tel").text(m.mobile===undefined?"--":m.mobile);o.find(".j-email").text(m.email===undefined?"--":m.email);o.find(".j-sign").text(m.sign===undefined?"--":m.sign);if(m.custom&&m.custom!=""){var j=JSON.parse(m.custom);o.find(".j-dept").text("部门："+j.department);o.find(".j-phone").text(j.mobilePhone===undefined?"--":j.mobilePhone)}if(p){o.find(".mutelist>.u-switch").addClass("off")}if(!n){o.find(".blacklist>.u-switch").addClass("off")}else{o.addClass("blacklist")}if(!l){o.addClass("notFriend")}else{var k=this.cache.getFriendAlias(m.account);o.find(".e-alias").val(k)}this.$mask.removeClass("hide");o.removeClass("hide")},hideInfoBox:function(){this.$personCard.addClass("hide");this.$mask.addClass("hide");this.$personCard.removeClass("notFriend");this.$personCard.removeClass("blacklist");this.$personCard.find(".mutelist .u-switch").removeClass("off");this.$personCard.find(".blacklist .u-switch").removeClass("off")},showCloudMsg:function(){var g=this;if(this.$chatEditor.data("type")==="team"){var k=this.$teamInfoBtn.data("team-id");var h=g.cache.getTeamById(k);if(!h){return}}$("#j-cloudMsgList").html("");this.$rightPanel.addClass("hide");this.$cloudMsgContainer.removeClass("hide");var i=g.$chatEditor.data("to"),l=g.$chatEditor.data("type"),j={scene:l,to:i,lastMsgId:0,limit:20,reverse:false,done:g.cbCloudMsg.bind(g)};g.mysdk.getHistoryMsgs(j)},closeCloudMsgContainer:function(){this.$rightPanel.removeClass("hide");this.$cloudMsgContainer.addClass("hide")},loadMoreCloudMsg:function(){var e=this.$chatEditor.data("to"),g=this.$chatEditor.data("type"),h=$("#j-cloudMsgList .item").first(),f={scene:g,to:e,beginTime:0,endTime:parseInt(h.attr("data-time")),lastMsgId:parseInt(h.attr("data-idServer")),limit:20,reverse:false,done:this.cbCloudMsg.bind(this)};this.mysdk.getHistoryMsgs(f)},cbCloudMsg:function(j,g){var i=$("#j-cloudMsgList"),h=$("#cloudMsg .u-status span");if(!j){if(g.msgs.length===0){h.html("没有更早的聊天记录")}else{if(g.msgs.length<20){h.html("没有更早的聊天记录")}else{h.html('<a class="j-loadMore">加载更多记录</a>')}var f=appUI.buildCloudMsgUI(g.msgs,this.cache);$(f).prependTo(i)}}else{console&&console.error("获取历史消息失败");h.html("获取历史消息失败")}},sendTextMessage:function(){var e=this.$chatEditor.data("type"),f=this.$chatEditor.data("to"),d=this.$messageText.val().trim();if(!!f&&!!d){if(d.length>500&&d.length===0){$.core.showErrorInfo("消息内容不能为空，且长度最大为500字符")}else{this.mysdk.sendTextMessage(e,f,d,this.sendMsgDone.bind(this))}}},sendMsgDone:function(d,f){this.cache.addMsgs(f);this.$messageText.val("");this.$chatContent.find(".no-msg").remove();var e=appUI.updateChatContentUI(f,this.cache);this.$chatContent.append(e).scrollTop(99999);$("#uploadFileForm").get(0).reset()},inputMessage:function(f){var e=f||window.event,d=$(this);if($.trim(d.val()).length>0){if(e.keyCode===13&&e.ctrlKey){d.val(d.val()+"\r\n")}else{if(e.keyCode===13&&!e.ctrlKey){im.sendTextMessage()}}}},doMsg:function(g){var j=this,f=g.to===userUID?g.from:g.to,h=function(){if($("#sendMsgContent").data("to")===f){var a=appUI.updateChatContentUI(g,j.cache);j.$chatContent.find(".no-msg").remove();j.$chatContent.append(a).scrollTop(99999)}};if(/text|image|file|audio|video|geo|custom/i.test(g.type)){this.cache.addMsgs(g);var i=(g.scene==="p2p"?f:g.from);if(!this.cache.getUserById(i)){this.mysdk.getUser(i,function(b,a){if(!b){j.cache.updatePersonlist(a);h()}})}else{this.buildConversations();h()}}else{notification.messageHandler(g,h)}},cbShowEmoji:function(h){if(!!h){var f=this.$chatEditor.data("type"),e=this.$chatEditor.data("to");if(h.type==="pinup"){var g=Number(h.emoji)+1;content={type:3,data:{catalog:h.category,chartlet:h.category+"0"+(g>=10?g:"0"+g)}};this.mysdk.sendCustomMessage(f,e,content,this.sendMsgDone.bind(this))}else{this.$messageText[0].value=this.$messageText[0].value+h.emoji}}},showEmoji:function(){this.initEmoji();this.$emNode._$show()},chooseFile:function(){this.$fileInput.click()},sendFile:function(){var g=this,f=g.$chatEditor.data("type"),e=g.$chatEditor.data("to"),h=this.$fileInput.get(0);if(h.files[0].size==0){$.core.showErrorInfo("不能传空文件");return}this.mysdk.sendFileMessage(f,e,h,this.sendMsgDone.bind(this))},playAudio:function(){if(!!window.Audio){var c=$(this),d=$(this).children(".j-play");c.addClass("play");setTimeout(function(){c.removeClass("play")},parseInt(d.attr("data-dur")));new window.Audio(d.attr("data-src")+"&audioTrans&type=mp3").play()}},convertAudio:function(){var d=$(this).parent().find(".j-play");var c=d.attr("data-src")},showLocation:function(){var b=$(this);$.core.open(ctx+"/pubs/map?longitude="+b.data("lng")+"&latitude="+b.data("lat")+"&info="+b.data("nick"),b.data("nick"))},showSysMsgCount:function(){var f=$("#systemNotice .count");var i=this.cache.getSysMsgCount();if(i>0){var g=$("#chatWindow",window.parent.document);if(g.hasClass("windowHidden")){var h=$("#eidpChat",window.parent.document);var j=h.find(".badge-count");if(!j.length){j=$("<span class='badge badge-count'></span>");h.append(j);j.text(i)}else{j.text(i)}}f.removeClass("hide").text(i)}else{f.addClass("hide").text(i)}},showSystemNotice:function(){var b=this;this.cache.setSysMsgCount(0);this.showSysMsgCount();if(this.firstLoadSysMsg){this.mysdk.getLocalSysMsgs(function(a,d){if(!a){if(d.sysMsgs.length>0){b.cache.setSysMsgs(d.sysMsgs)}b.firstLoadSysMsg=false;b.showNotice()}else{alert("获取系统消息失败")}})}else{this.showNotice()}},showNotice:function(){var b=this;$.core.backgroundRequest(ctx+"/core/personal/notification/0/find?page="+page+"&rows=10",{type:"GET",callback:function(e){if(e&&e!=null){var f=[];$.each(e,function(h,d){var c={time:d.sendTime,content:JSON.parse(d.extraData)};f.insert(0,c)});if(f.length>0){b.$rightPanel.addClass("hide");b.$notice.removeClass("hide");b.$teamInfoContainer.addClass("hide");b.$cloudMsgContainer.addClass("hide");b.$conversations.find(".active").removeClass("active");var a=b.buildSysMsgs(f);b.$notice.find("#sysNoticeContent").html(a);$("#sysNoticeContent").scrollTop(9999)}else{b.$notice.find("#sysNoticeContent").html("暂无系统消息哦...")}}else{b.$notice.find("#sysNoticeContent").html("暂无系统消息哦...")}}})},buildSysMsgs:function(f){var d="";if(f.length===0){return d}f=f.sort(function(a,b){return b.time-a.time});for(var e=0;e<f.length;e++){content=JSON.parse(f[e].content);d+='<p class="u-msgTime"><code>'+transTime(f[e].time)+"</code></p>";d+='<div class="panel panel-default"><div class="panel-heading">'+content.relBizTypeName+'提醒</div><div class="panel-body"> '+content.desc+"<blockquote>"+content.relBizSubject+"</blockquote></div></div>"}return d},loginPorts:function(h){var f,g;for(var e=h.length-1;e>=0;e--){if(/iOS|Android|WindowsPhone/i.test(h[e].type)){g=h[e]}else{if(/PC/i.test(h[e].type)){f=h[e]}}}if((f&&f.online)||(g&&g.online)){if((f&&f.online)&&(g&&g.online)){$(".m-devices").html("正在使用云信手机版，电脑版");$("#j-devices .pc").removeClass("hide");$("#j-devices .mobile").removeClass("hide");this.mysdk.mobileDeviceId=g.deviceId;this.mysdk.pcDeviceId=f.deviceId}else{if(f&&f.online){$(".m-devices").html("正在使用云信电脑版");$("#j-devices .pc").removeClass("hide");$("#j-devices .mobile").addClass("hide");this.mysdk.mobileDeviceId=false;this.mysdk.pcDeviceId=f.deviceId}else{$(".m-devices").html("正在使用云信手机版");$("#j-devices .mobile").removeClass("hide");$("#j-devices .pc").addClass("hide");this.mysdk.mobileDeviceId=g.deviceId;this.mysdk.pcDeviceId=false}}$(".m-devices").removeClass("hide");$(".friends").height(463);$("#j-chatVernier").css({marginTop:"41px"})}else{$(".m-devices").addClass("hide");$("#j-devices").addClass("hide");$("#j-devices .pc").addClass("hide");$("#j-devices .mobile").addClass("hide");this.mysdk.mobileDeviceId=false;this.mysdk.pcDeviceId=false;$(".friends").height(504);$("#j-chatVernier").css({marginTop:"0"})}},showDevices:function(){this.$devices.removeClass("hide")},hideDevices:function(){this.$devices.addClass("hide")}};
