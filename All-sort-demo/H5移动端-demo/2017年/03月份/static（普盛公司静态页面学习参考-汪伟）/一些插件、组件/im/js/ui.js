var appUI = {
        ...
        if (message.attach && message.attach.type) {
            ...
            var type = message.type,
                ...
                msgHtml = ['<div data-time="' + message.time + '" data-id="' + message.idClient + '" data-idServer="' + message.idServer + '" class="item item-' + buildSender(message) + '">',
                    '<img class="img j-img" src="' + getAvatar(avatar,user.gender) + '" data-account="' + from + '"/>',
                    ...
                    '</div>',
                    '</div>',
                    message.status === "fail" ? '<span class="error"><i class="im-icon imicon-error"></i>发送失败</span>' : '',
                    '</div>'].join('');
        }
        return msgHtml;
    ...
    buildTeamMemberList: function (list) {
        return ['<li data-icon="' + list.avatar + '" data-uid="' + list.account + '" data-account="' + list.nick + '">',
            '<i class="im-icon imicon-radio"></i>',
            '<img src="' + getAvatar(list.avatar,list.gender) + '">',
    ...
    buildBlacklist: function (data, cache) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            html += ['<li class="items f-cb">',
                '<img src="' + getAvatar(user.avatar,user.gender) + '" class="head">',
    ...
    buildCustomSysMsgs: function (data, cache) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (scene === "p2p") {
                nick = getNick(from);
                var user = cache.getUserById(from);
                if(user) {
