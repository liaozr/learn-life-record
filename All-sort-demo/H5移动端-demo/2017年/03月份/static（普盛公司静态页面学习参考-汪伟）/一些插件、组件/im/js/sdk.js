 ...
 * SDK连接 功能相关
 */
var SDKBridge = function (ctr, data, appKey, userUID, sdktoken) {
    var that = this;
    //缓存需要获取的用户信息账号
    this.person = {};
    //缓存需要获取的群组账号
        ...
        if (error) {
            switch (error.code) {
                ...
                case 302:
                    ...
                    top.$("#chatWindow").data("kendoWindow").close();
                    break;
                // 被踢, 请提示错误后跳转到登录页面
                case 'kicked':
                    var map = {
                    ...
                    var str = error.from;
                    $.core.showErrorInfo("您的IM帐号于" + dateFormat(+new Date(), "HH:mm") + "被" + (map[str] || "其他端") + "踢出下线，请确定帐号信息安全!");
                    top.$("#chatWindow").data("kendoWindow").close();
                    break;
                default:
                    break;
...
SDKBridge.prototype.kick = function (type) {
    ...
    this.nim.kick({
        deviceIds: [deviceIds],
            ...
            $.core.showErrorInfo('踢' + (type === 0 ? '移动' : 'PC') + '端' + (!error ? '成功' : '失败'));
            console.log(error);
            console.log(obj);
        }
    });
