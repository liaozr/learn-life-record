        ...
        case 'custom':
            try{
                ...
                if (content.type === 1) {
                    ...
                    text += '['+content.relTypeName+']'+content.relNo;
                } else {
                    text += '[自定义消息]';
                }
            } catch(e){
    ...
    switch (msg.type) {
        ...
        case 'image':
            if (msg.status === -1) {
                ...
                var group = msg.from + "-" + msg.to;
                msg.file.url = _$escape(msg.file.url);
                str = '<a class="fancybox" data-fancybox-group="' + group + '" href="' + msg.file.url + '?imageView" target="_blank"><img onload="loadImg()" data-src="' + msg.file.url + '" src="' + msg.file.url + '?imageView&thumbnail=200x0&quality=85"/></a>';
            }
            break;
        case 'file':
            if (msg.status === -1) {
        ...
        case 'audio':
            if (!!window.Audio) {
                if (msg.from === userUID && msg.from !== msg.to) {
                    str = '<div class="u-audio j-mbox right"> <a href="javascript:;" class="j-play playAudio" data-dur="' + msg.file.dur + '"  data-src="' + url + '">点击播放</a><b class="j-duration">' + Math.floor((msg.file.dur) / 1000) + '"</b><span class="u-icn u-icn-play" title="播放音频"></span></div><span class="convert icon-refresh"></span>'
                } else {
                    str = '<div class="u-audio j-mbox left"> <a href="javascript:;" class="j-play playAudio" data-dur="' + msg.file.dur + '"  data-src="' + url + '">点击播放</a><b class="j-duration">' + Math.floor((msg.file.dur) / 1000) + '"</b><span class="u-icn u-icn-play" title="播放音频"></span></div><span class="convert icon-refresh"></span>'
                }
            } else {
        ...
        case 'geo':
            var geo = msg.geo;
            str = '<img class="location" data-nick="'+msg.fromNick+'" data-lng="'+geo.lng+'" data-lat="'+geo.lat+'" src="/view/static/comp/im/images/location_bg.png"><div class="locationTitle">'+geo.title+'</div>';
            break;
        case 'custom':
            try{
                var content = JSON.parse(msg.content);
                if (content.type === 1) {
                    ...
                    str = '<img class="chartlet" onload="loadImg()" src="/view/static/comp/im/images/' + catalog + '/' + chartlet + '.png">';
                } else if (content.type === 4) {
                    str = sentStr + '一条[白板]消息,请到手机或电脑客户端查看';
                } else if (content.type === 5) {
                    str = "<div class='shareMsg' onclick='showShareBill(\""+content.relType+"\",\""+content.relTypeName+"\",\""+content.relNo+"\")'><div class='shareMsg-heading'><i class='icon-link'/>"+content.relTypeName+"</div>";
                    str += "<div class='shareMsg-body'>"+content.relNo+"</div>";
                    str += "</div>";
                } else {
...
function transNotification(item) {
    ...
    if (!team) {
        team = im.cache.getTeamMapById(item.target);
    } else {
        if (!team.type) {
            team = im.cache.getTeamMapById(item.target);
        }
...
function getAvatar(url,gender) {
    if(!url || url == ""){
        if(gender == 2) {
            return ctx + "/view/static/comp/im/images/felman.png";
        }
        return ctx + "/view/static/comp/im/images/man.png";
    }
